...
namespace _4TAX_Service.Application.Client
{
    public class NFSeProcess : NFSeHandler
    {
        ...
        public void IntegrateNFSe(List<NFSeB1Object> ListNFSe, EmitMapper emitMapper, Emit emit)
        {
            ...
            foreach (var item in ListNFSe)
            {
                try
                {
                    ...
                    {
                        UpdateStatusNFSeB1Sucess(response, item.DocEntry, item.BPLId, item.NumeroLote);
                    }
                    else
                    {
        ...
        public bool UpdateStatusNFSeB1Sucess(OperationResponse<EmitSuccessResponseOutput, EmitFailResponseOutput> response, int DocEntry, int BPLId, int NumeroLote)
        {
            DataBaseNFSeProcess dataBaseNFSeProcess = new DataBaseNFSeProcess(dbWrapper);
            ...
            if(NumeroLote > 0)
            {
                dataBaseNFSeProcess.UpdateNFSeODBC($@"UPDATE ""@TAX4_CONFIG"" SET ""U_TAX4_NumLote"" = '{NumeroLote + 1}' WHERE ""U_TAX4_Filial"" = '{BPLId}' ");
            }
