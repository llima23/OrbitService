...
namespace B1Library.Documents.Entities
{
    public class SetupQueryB1
    {
		...
		public SetupQueryB1(DBDocumentsRepository dbRepo)
        {
			this.dbRepo = dbRepo;
        }
        ...
        public string ReturnCommandB1ConsultDocumentInOrbit()
        {
            ...
            sb.AppendLine($@"SELECT
							 ...
							 COALESCE(CO.""U_TAX4_CamXML"",'')		 AS ""CaminhoXML"",
						     COALESCE(CO.""U_TAX4_DownloadAuto"",'') AS ""DownloadAutomatico"",
							 COALESCE(CO.""U_TAX4_EnvEm"",'')		 AS ""EnviaEmailAutomatico""
							 FROM {B1TableName} T0
							 JOIN ONFM OM ON T0.""Model"" = OM.""AbsEntry""
							 JOIN ""@TAX4_CONFIG"" CO ON T0.""BPLId"" = CO.""U_TAX4_Filial""
							 JOIN {B1TableNameChild}1 T1 ON T0.""DocEntry"" = T1.""DocEntry""
        ...
        public string ReturnCommandHeader()
        {
            ...
            sb.AppendLine($@"FROM {B1TableName} T0  
							JOIN ONFM OM ON T0.""Model"" = OM.""AbsEntry""
							JOIN ""@TAX4_CONFIG"" CO ON T0.""BPLId"" = CO.""U_TAX4_Filial""
							JOIN ""@TAX4_LCONFIGADDON"" CG ON T0.""BPLId"" = CG.""U_TAX4_Empresa""
							LEFT JOIN NFN1 NF ON T0.""SeqCode"" = NF.""SeqCode""");
            sb.AppendLine(useCasesB1.GetCommandUseCase());
            sb.AppendLine(@"AND T0.""DocDate"" >= CO.""U_TAX4_DateInt""");
            return Convert.ToString(sb);
        }
		public string ReturnConfigEmailSMTP()
        {
			sb = new StringBuilder();
			sb.AppendLine($@"SELECT
                            COALESCE(CG.""U_TAX4_EmNomeExib"",'') AS ""NomeDeExibicao"",
                            COALESCE(CG.""U_TAX4_EmSaida"",'') AS ""EmailRemetente"",
                            COALESCE(CG.""U_TAX4_EmSMTP"",'') AS ""SMTP"",
                            COALESCE(CG.""U_TAX4_EmPorta"",0) AS ""PortaSMTP"",
                            COALESCE(CG.""U_TAX4_EmUsuario"",'') AS ""UsuarioSMTP"",
                            COALESCE(CG.""U_TAX4_EmSenha"",'') AS ""SenhaSMTP"",
                            COALESCE(CG.""U_TAX4_EmAssunto"",'') AS ""AssuntoEmail"",
                            COALESCE(CG.""U_TAX4_EmTexto"",'') AS ""CorpoDeEmail"",
                            COALESCE(CG.""U_TAX4_EmAutSMTP"",'') AS ""AutenticacaoSMTP"",
                            COALESCE(CG.""U_TAX4_EmCripSSL"",'') AS ""CriptografiaSSL"",
                            COALESCE(CG.""U_TAX4_EmCopiaCont"",'') AS ""EnviaEmailContato"",
                            COALESCE(CG.""U_TAX4_EmCopiaOcul"",'') AS ""EnviaEmailOculto"",
                            COALESCE(CG.""U_TAX4_EmAutNFSe"",'') AS ""EnviaEmailAutNFSe"",
                            COALESCE(CG.""U_TAX4_EmCancelNFSe"",'') AS ""EnviaEmailCancelNFSe"",
                            COALESCE(CG.""U_TAX4_EmAutNFe"",'') AS ""EnviaEmailAutNFe"",
                            COALESCE(CG.""U_TAX4_EmCancelNFe"",'') AS ""EnviaEmailCancelNFe""
							FROM ""@TAX4_CONFIGADDON"" CG
							");
			return Convert.ToString(sb);
        }
        public string ReturnCommandIdentificacao(Invoice invoice)
        {
            ...
            sb.AppendLine($@"SELECT
									 ...
									 COALESCE(OI.""Comments"",'')				   AS ""ObsDocumento"",
									 ...
									 COALESCE(C2.""U_TAX4_TpObs"",0)			   AS ""TipoObservacao""
									 FROM {B1TableName} OI 
        ...
        public string ReturnCommandParceiroHANA1(Invoice invoice)
        {
            ...
            if (B1TableType == DBTableNameRepository.Type.Saida)
            {
                sb.AppendLine(@$"SELECT
								...
								WHERE PT.""DocEntry"" = {invoice.DocEntry} 
								and D1.""AdresType"" = 'S'");
            }
            else
            {
        ...
        public string ReturnCommandImpostoLinha(CabecalhoLinha cabecalhoLinha)
        {
			...
			//VBcUfDest
			if (VerifyIfFieldExists("VBcUfDestL"))
			{
				sb.AppendLine(@$"	 ,COALESCE(TX.""U_VBcUfDestL"",0)		   AS ""vBCUFD""");
			}
			//VBcFcpUfDest
			if (VerifyIfFieldExists("VBcFcpUfDestL"))
			{
				sb.AppendLine(@$"	 ,COALESCE(TX.""U_VBcFcpUfDestL"",0)		   AS ""vFCUFD""");
			}
			//VIcmsUfDest
			if (VerifyIfFieldExists("VIcmsUfDestL"))
			{
				sb.AppendLine(@$"	 ,COALESCE(TX.""U_VIcmsUfDestL"",0)		   AS ""vICUFD""");
			}
        ...
        public string ReturnCommandRetencoesLinha(CabecalhoLinha cabecalhoLinha)
        {
            ...
            sb.AppendLine(@$"SELECT
									 ...
									 COALESCE(OH.""OffclCode"",'')		   AS ""CodOfiReceita"",
									 OH.""WTTypeId"" AS ""TipoImpostoOWHT""
									 FROM {B1TableNameChild}5 WT
									 INNER JOIN OWHT OH on WT.""WTCode"" = OH.""WTCode""
									 LEFT JOIN OWTT OW on OH.""WTTypeId"" = OW.""WTTypeId""
		...
		public bool VerifiyIfMultBranch()
        {
			DataSet queryResult = dbRepo.wrapper.ExecuteQuery(@$"SELECT ""MltpBrnchs"" FROM OADM");
			string MltpBrnchs = queryResult.Tables[0].Rows[0].ItemArray[0].ToString();
			if (MltpBrnchs == "Y")
			{
				return true;
			}
